...
#!!! Find optimal K!!!!
#it seems that 4 can be the optimal K - where the MCE for validation data is minimal and model with k=4 i simpler

#estimate the test error for the model with optimal k , compare it with the training and validation
modelTest = kknn(V65~., train, test, na.action = na.omit(),k = 4, kernel = "rectangular", scale=TRUE)
MCETest=mean(modelTest$fitted.values != test$V65) 
...
for (j in 1:30) {
   ...
   for (i in 1:length(valid$V65)){
     for (m in 1:10){
      CrossEntropyValid = CrossEntropyValid + as.numeric(as.numeric(valid$V65[i])==as.numeric(C[m])) * log(as.double(modelValid1$prob[i,m])+10^-5)
...
matplot(K,cbind(vectorValidMCE,vectorTrainMCE,vectorValidCrossEntropy),type="l",col=c("red","blue","purple"),lty=c(1,1))
#matpoints(5,MCETest,col="green")
legend(15,0.31,legend=c("Valid-MCE","Train-MCE","Valid-CrossEntrpy"),col=c("red","blue","purple"),pch=c(16,1))
minor.tick(nx = 5,  tick.ratio = 1)
